...
/* ---------- Logging window ---------- */
constexpr uint32_t LOG_WINDOW_MAX_MS = 10000;   // for on-screen freshness
...
/* ---------- Ping queue (captures *every* BLE adv) ---------- */
struct PingEvent {
  uint8_t  devIdx;
  int16_t  rssi;
  uint32_t ms;
...
PingEvent        q[QUEUE_SZ];          // no volatile here
volatile size_t  qHead = 0, qTail = 0; // only indices are volatile
