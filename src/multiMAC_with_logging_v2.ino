...
/* ---------- Logging window ---------- */
constexpr uint32_t LOG_WINDOW_MAX_MS = 10000;
...
uint32_t lastWrittenMs[NUM_DEV] = {0};  // Track last write per device
...
void loop() {
  ...
  for (size_t i = 0; i < nRows; i++) {
    ...
    int16_t rssi = (millis() - devices[devIdx].lastSeenMs > LOG_WINDOW_MAX_MS) ? -128 : devices[devIdx].rssi;
  ...
  if (millis() - lastLog > 1000 && time(nullptr) > 1577836800) {
    ...
    if (f) {
      for (size_t i = 0; i < NUM_DEV; i++) {
        if (millis() - devices[i].lastSeenMs <= LOG_WINDOW_MAX_MS &&
            devices[i].lastSeenMs > lastWrittenMs[i]) {
          lastWrittenMs[i] = millis();
          String row = now + "," + ssid + "," + devices[i].macStr + "," + String(devices[i].rssi);
          fputs(row.c_str(), f);
          fputs("\n", f);
        }
      }
